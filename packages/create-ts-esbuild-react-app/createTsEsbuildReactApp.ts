...
function init() {
  const program = new commander.Command(packageJson.name)
    ...
    .on("--help", () => {
      ...
      console.log(
        `      ${chalk.cyan(
          "https://github.com/nathanjhood/ts-esbuild-react/issues/new"
        )}`
      );
      console.log();
    })
      ...
      .run(
        {
          ...
          npmGlobalPackages: ["ts-esbuild-react"], // ["create-react-app"],
        },
        {
          duplicates: true,
          showNotFound: true,
  ...
  checkForLatestVersion()
    .catch(() => {
      try {
        return execSync("npm view ts-esbuild-react version").toString().trim();
      } catch (e) {
        return null;
      }
    })
    .then((latest) => {
      if (latest && semver.lt(packageJson.version, latest)) {
        ...
        console.error(
          chalk.yellow(
            `You are running \`ts-esbuild-react\` ${packageJson.version}, which is behind the latest release (${latest}).\n\n` +
              "We recommend always using the latest version of ts-esbuild-react if possible."
          )
        );
        console.log();
        console.log(
...
function createApp(name, verbose, version, template, useYarn, usePnp) {
  ...
  console.log(
    `Creating a new esbuild-powered React app in ${chalk.green(root)}.`
  );
  console.log();
...
function getTemplateInstallPackage(template, originalDirectory) {
  let templateToInstall = "ts-esbuild-react-template";
  if (template) {
...
function checkAppName(appName) {
  ...
  const dependencies = ["react", "react-dom", "ts-ebuild-react-scripts"].sort();
